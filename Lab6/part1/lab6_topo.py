  ...
  def __init__(self):
    ...
    def add_default_routes(self):
        for host in self.hosts():
            # Get the switch this host is connected to
            switch = host.defaultIntf().link.intf2.node
            # Add default route via the switch
            host.cmd('route add default gw %s' % switch.IP())
...
if __name__ == '__main__':
  ...
  for host in net.hosts:
    host.cmd('route add default dev %s-eth0' % host.name)
