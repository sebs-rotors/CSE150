...
class Routing (object):
  ...
  def do_routing (self, packet, packet_in, port_on_switch, switch_id):
    ...
    if packet.find('icmp') is not None:
        ...
        same_subnet = (ipaddress.ip_address(src_ip) in subnets[0] and ipaddress.ip_address(dst_ip) in subnets[0]) or \
                      (ipaddress.ip_address(src_ip) in subnets[1] and ipaddress.ip_address(dst_ip) in subnets[1]) or \
                      (ipaddress.ip_address(src_ip) in subnets[2] and ipaddress.ip_address(dst_ip) in subnets[2])
